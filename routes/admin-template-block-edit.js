...
module.exports = function(app) {
  ...
  app.post('/admin-template-block-edit', function (req, res) {
    // get query string
    var codes = []
    ...
    pool.connect(function (err, client, done) {
      ...
      dbClient = client
      dbClient.query('UPDATE block SET name=$1, description=$2, start_time=$3, end_time=$4, proc_codes=$5 where id=$6;',[name, description, start_time,end_time,proc_codes,id], function (err, result) {
        if (err) {
          console.log("Error UPDATING data to table block. " + err)
        }
        if(result) {
